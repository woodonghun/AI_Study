...
model_save_path = r'D:\AI 공부\1. 보스턴 집값\model'
...
class Tensordata(Dataset):
    def __init__(self, file_path):
        ...
        self.x = dataframe.drop('PRICE', axis=1).to_numpy()
        scaler_minmax.fit(self.x)
        self.x = scaler_minmax.transform(self.x)
        self.x = torch.FloatTensor(self.x)

        self.y = dataframe['PRICE'].to_numpy().reshape((-1, 1))  # 타겟
...
dataset = Tensordata(r'D:\AI 공부\1. 보스턴 집값\boston.csv')
...
for epoch in range(epoch_size):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        predict_outputs = predict_outputs.detach()  # gradient 의 전파를 멈추는 역할/ 사용 하지 않으면 mean_squared_error 함수 사용 불가
    ...
    if loss_save < ls:
        ...
        torch.save(model, f'{model_save_path}\just_save')
    # Tensorboard 에 저장
