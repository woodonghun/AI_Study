...
epoch_size = 5000
weight_decay = 1e-7
project_name = 'titanic_survived'

model_save_path = r'D:\AI 공부\7. titanic\model'
...
class Tensordata(Dataset):
    def __init__(self, file_path):
        ...
        dataframe = dataframe.drop(['PassengerId', 'Embarked', 'Ticket', 'Fare', 'Cabin', 'Name'], axis=1)  # data
        print(dataframe.shape)
        print(dataframe.count())

        dataframe = dataframe.dropna()
        print(len(dataframe))
        self.y = dataframe['Survived'].to_numpy().reshape((-1, 1))  # target
        # scaler_minmax.fit(self.y)
        # self.y = scaler_minmax.transform(self.y)
        self.y = torch.FloatTensor(self.y)

        self.x = dataframe.drop(['Survived'], axis=1)  # data
        self.x = self.x.replace(to_replace=['male', 'female'], value=[1, 0])  # male = 1, female = 0
...
dataset = Tensordata(r'D:\AI 공부\7. titanic\all_data.csv')
...
class Model(nn.Module):  # 모델
    def __init__(self):
        ...
        self.fc1 = nn.Linear(5, 20, bias=True)
    ...
    def forward(self, x):  # 모델 연산의 순서를 정의
        ...
        x = torch.sigmoid(self.fc2(x))  # Linear 계산 후 활성화 함수 ReLU를 적용한다.
...
checkpoint = torch.load(r'D:\AI 공부\7. titanic\model\temp.pt')
model.load_state_dict(checkpoint['model'])
print(model)
model.eval()
with torch.no_grad():
